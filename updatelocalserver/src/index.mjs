...
export const getAndStorePredictions = async (state, district, commodity) => {
  try {
    ...
    // Fetch predictions (replace `getPredictions` with your actual prediction logic)
    const result = await getPredictions(state, district, commodity);
    console.log("Fetched predictions:", result);

    // Update if exists or create a new one
    ...
    if (existingPredictions) {
      ...
      console.log("Prediction stored successfully:", newPrediction);
    }
...
// await getAndStorePredictions("Bihar", "Araria", "Cauliflower");
